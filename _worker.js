...
const PATRONATY_TSV =
  "https://docs.google.com/spreadsheets/d/e/2PACX-1vQWKQFq5K4oAH8vFXeXxjZl4ym6rapVWN05oDhx7tykewL3A_FGQZcnk05XG0Va_e_cHWORpfqPs13m/pub?gid=1252632175&single=true&output=tsv";
...
function buildPatronatyHTML(tsv) {
  ...
  for (const year of years) {
    ...
    for (const r of byYear.get(year)) {
      ...
      if (cover) {
        html += `<img class="patronat-okladka" src="${esc(cover)}" alt="Okładka: ${esc(r[iTytul])}" loading="lazy">`;
      }
      html += `<div class="patronat-tresc">
        <h3 class="patronat-tytul">${esc(r[iTytul])}</h3>
        <p class="patronat-autor">${esc(r[iAutor])}</p>
        <p class="patronat-opis">${esc(r[iOpis])}</p>
      </div></article>`;
    }
    html += `</div></section>`;
  }
...
export default {
  async fetch(request, env, ctx) {
    ...
    // Sprawdzmy, czy to podstrona "Patronaty"
    if (url.pathname === "/patronaty/" || url.pathname === "/patronaty") {
      const page = await env.ASSETS.fetch(request);

      const tsvResp = await fetch(PATRONATY_TSV, {
        cf: { cacheTtl: 300, cacheEverything: true } // cache 5 minut, jak przy bio
      });
      if (!tsvResp.ok) return page; // awaria arkusza -> strona z komunikatem zapasowym
